...
export function calculateProvisionedCosts() {
    ...
    cfg.provisionedBaselineWCUHours = Math.ceil(cfg.provisionedBaselineWCU * cfg.baselineHoursWrites);
    ...
    cfg.provisionedBaselineRCUHours = Math.ceil(cfg.provisionedBaselineRCU * cfg.baselineHoursReads);
...
function logCosts() {
    ...
    if (cfg.pricing === 'demand') {
        ...
        if (cfg.dynamoCostDemandMonthlyReplicatedWRU !== 0) {
            logs.push(`Monthly write cost (replicated): ${Math.floor(cfg.dynamoCostDemandMonthlyReplicatedWRU).toLocaleString()}`);
        }
